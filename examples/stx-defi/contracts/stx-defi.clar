...
(define-public (borrow (amount uint))
    (let (
        ...
        (map-set loans tx-sender { amount: new-loan, last-interaction-block: tenure-height })
        ;; Return success.
        (ok true)
    )
)
...
(define-public (repay (amount uint))
    (let (
        ...
        (map-set loans tx-sender { amount: (- total-due amount), last-interaction-block: tenure-height })
...
(define-private (calculate-accrued-interest (principal uint) (start-block uint))
    (let (
        ...
        (elapsed-blocks (- tenure-height start-block))
